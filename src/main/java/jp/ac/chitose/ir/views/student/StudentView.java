...
public class StudentView extends VerticalLayout {
    ...
    private void comboBoxInitialyze() {
        ...
        comboBox.addValueChangeListener(e1 -> {
            ...
            histData.forEach(e2 -> {
                if(e2.成績評価().equals("秀")) data[4] = new Data<>(e2.成績評価(), e2.度数());
                else if(e2.成績評価().equals("優")) data[3] = new Data<>(e2.成績評価(), e2.度数());
                else if(e2.成績評価().equals("良")) data[2] = new Data<>(e2.成績評価(), e2.度数());
                else if(e2.成績評価().equals("可")) data[1] = new Data<>(e2.成績評価(), e2.度数());
                else data[0] = new Data<>(e2.成績評価(), e2.度数());
            });
            String[] strs = new String[]{"不可", "可", "良", "優", "秀"};
            for(int i = 0; i < 5; i++) if(data[i] == null) data[i] = new Data<>(strs[i], 0);
            final GraphSeries<Data<String, Integer>> series = new GraphSeries<>(e1.getValue().科目名());
            series.setData(data);
